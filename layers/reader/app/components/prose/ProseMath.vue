...
watch(
  [containerRef, () => props.content, isInline],
  () => nextTick(() => renderToEl(containerRef.value, props.content)),
  { immediate: true },
)
</script>
...
<template>
  <div v-if="!isInline" class="group relative my-5">
    <UButton
      ...
      class="absolute top-[11px] right-[11px] opacity-0 transition group-hover:opacity-100"
      tabindex="-1"
      @click="copy(content ?? '')"
    />
