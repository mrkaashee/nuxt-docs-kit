/**
 * useReadingStats — computes word count, character count, and estimated reading time
 * from a plain-text or markdown string.
 *
 * Reading speed: 238 wpm (average adult silent reading speed).
 */
...
export function computeReadingStats(rawText: string): ReadingStats {
  ...
  if (words === 0) readingTimeLabel = "—"
  else if (readingTimeMin < 1) readingTimeLabel = "< 1 min read"
  else readingTimeLabel = `${readingTimeMin} min read`
